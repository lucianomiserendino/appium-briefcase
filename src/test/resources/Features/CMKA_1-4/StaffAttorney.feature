Feature: Login as a Staff attorney: Assignment Categories display on the dashboard for SAs, Referral categories display for SAs , Verify Document Categories display 
...
grouped based on referral categories.  Each referral category is displayed as a collapsible panel.  This task is to verify each referral category is
 displayed and the number of referrals is correct for each category.
	If selecting a referral, the documents display on the referral detail page.  They are grouped by document category.  
	 Each document category is a collapsible panel.  This task is to verify the correct document categories and number of docs
	  in each category display for a selected referral. 
	...
	Given User selects assignment type "<assignmentType>" 
	When User observes there are six referral categories listed. 
	Then User  selects  "<category>" and  "<case>" 
	And User verifies the document categories and the number of docs displayed for each category after selecting "<category>"  and "<dbType>" 
	
	
	Examples: 
		|environment   |userName             |password  |server                          |dbType |assignmentType       |category    |case   |
		|Integration   |KristenStaffAttorney |Test2022!|Appellate DC Development - CMKA  |CMKA   |Senior Staff Attorney|Anders Cases|15-3015|
