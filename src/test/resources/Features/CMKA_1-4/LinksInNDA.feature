...
Scenario Outline: 
	...
	User's can set preferences for the Notices of Docket Activity (NDA) they receive to include links to open documents, 
    notes and case information in Briefcase (see attachment).  When these links are clicked, the item will open in Briefcase rather
	...
	Then Verify the document opens in briefcase by using the same "<case>" and "<dbType>" (Open a document in Briefcase from the NDA link). 
	Examples: 
