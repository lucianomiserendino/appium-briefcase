...
removes any terminated document and retrieves new document information after a specified amount of time that is stored in the site table
 variable briefcaseAutoSyncMinutes.  This task is to automate the autosync feature for new and terminated referrals.
	...
	Then User  sets the chambers_case_to_referral.ccr_date_end date field to today's date  in "<dbType>" , using  "<peID>"  , indicating the referral "<caseNum>" is terminated. 
	...
	Examples: 
		|environment   |userName          |password  |server                             |refCategory       | caseNum  |dbType|peID|
		|Integration   |judge werner      |Test2019! |Appellate DC Development - CMKA    |Test Automation   | 15-2594  |CMKA  |34  |
