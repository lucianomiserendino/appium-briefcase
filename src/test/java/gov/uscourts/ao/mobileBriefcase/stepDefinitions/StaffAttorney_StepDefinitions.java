...
public class StaffAttorney_StepDefinitions {
	...
	public void user_observes_correct_referral_categories_listed_for_Staff_Attorney() {
		List<UserInputData> userInputData = null;
		page = new StaffAttorneyReferralPage();
		page.observeReferralCategories(userInputData);
	}
	...
	public void user_selects_category_and_SMR_ASSIGN_PE_ID() {
		page = new StaffAttorneyReferralPage();
		List<UserInputData> userInputData = null;
		page.tapOnReferralCategory(userInputData);
	}

	@Then("^User verifies the document categories and the number of docs displayed for each category matches the number of docs in DB\"$")
