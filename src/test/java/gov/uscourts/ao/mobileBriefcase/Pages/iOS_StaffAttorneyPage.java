...
public class iOS_StaffAttorneyPage extends AppiumPageFactory {
			...
			for (int i = 0; i < category.size(); ++i) {
				...
				for (int j = 0; j < refCatId.size(); j++) {
					...
					for (int k = 0; k < refNumbers.size(); k++) {
						...
						MobileElement cat = findElementBy(Locator.XPATH, containsElement(category.get(i)));

						if (!cat.getText().contains("("))
							cat.click();

						MobileElement uiResult = findElementBy(Locator.XPATH,
								containsElement(category.get(i) + " (" + refNumbers.get(k) + ")"));
	...
	public void getCategories(DBType dbType, String referral) {
		elementIsDisplayed(dbType, replace(DOCUMENT_CATEGORIES, "SMR_MRC_ID", getDocumentCategories(dbType, referral)),
				"//XCUIElementTypeStaticText");
		assertTrue(getDocuments(referral, dbType));
	}

	public boolean getDocuments(String referral, DBType dbType) {
