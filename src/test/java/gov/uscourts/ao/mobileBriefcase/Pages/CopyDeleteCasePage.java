...
public class CopyDeleteCasePage extends AppiumPageFactory {
	...
	// findElementsAndsendKeys(By.name(CASE_NUMBER_OF_THE_CASE_TO_BE_COPIED), 0,
	// getProperty(CASE));
	// findElementAndsendKeys(By.name(CASE_NUMBER_TO_BE_USED_FOR_NEW_CASE),
	// caseNum);
	// findElementAndclick(By.xpath(COPY_CASE));
	// confirm(CONFIRM_COPY_PAGE, CONFIRM_COPY, CONFIRM_COPY_BTN);
	// assertEquals("******CASE CANNOT BE COPIED, PLEASE ENTER A VALID CASE
	// NUMBER******",
	// findWebElement(By.xpath(CASE_INFO_REPORT)).getText(),
	// (COPY_CASE_INFO_REPORT));
	// closeWebDriver();
	//
	...
	// String case_date_entered = findElementThatcontains("cs_date_entered",
	// 2).getAttribute("value");
	// findWebElement(By.xpath(CS_DATE_ENTERED)).clear();
	// findWebElement(By.xpath(CS_DATE_ENTERED))
	// .sendKeys(case_date_entered.replace(case_date_entered.substring(2, 4),
	// splitBy(caseNum, 0)));
	// findElementThatcontains("update1").click();
	//
	// assertEquals("*******CASE CANNOT BE UPDATED, PLEASE ENTER A VALID CASE
	// NUMBER********",
	// findWebElement(By.xpath(UPDATED_RECORD)).getText().trim(),
	// (UPDATED_RECORD_RESULT));
	// closeWebDriver();
	//
	...
	// getURLandLogin(COPY_DEL_CASE);
	// findElementsAndsendKeys(By.name(CASE_NUMBER_OF_THE_CASE_TO_BE_COPIED), 1,
	// caseN);
