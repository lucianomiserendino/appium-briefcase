...
public class DocumentPage extends AppiumPageFactory {
	...
	public static void selectRandomItem(String query, String xpath, List<UserInputData> userInputData) {
	...
	public static void selectRandomSTFCategory(List<UserInputData> userInputData) {
		...
		selectRandomItem(Actions.replace(Queries.SAs_ASSIGNMENT_CATEGORIES, "RA_PE_ID", "434"), xpath,
				userInputData);

	}

	public static void selectRandomJudgeCategory(List<UserInputData> userInputData) {

		String name = SystemPropertySetup.getJudge(userInputData);
		selectRandomItem(getID(Queries.REFERRAL_CATEGORIES, getPE_ID("jud", name, userInputData)), xpath,
				userInputData);
	...
	public  void selectRandomCaseNumber(List<UserInputData> userInputData) {
		...
		MobileElement uiResult = findElementBy(Locator.XPATH,
				"//XCUIElementTypeStaticText[contains(@name, '" + list.get(Utility.getRandomInt(list.size() - 1)) + "')]");
		uiResult.click();
